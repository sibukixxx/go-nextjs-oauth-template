import { useState, useEffect } from 'preact/hooks';
import { isAuthenticated, setAuth } from '@/signals/auth';
import { api } from '@/lib/api';

interface Provider {
  name: string;
  type: string;
}

export function LoginPage() {
  const [providers, setProviders] = useState<Provider[]>([]);
  const [loading, setLoading] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    if (isAuthenticated.value) {
      window.location.href = '/';
    }
  }, []);

  useEffect(() => {
    api
      .get<{ providers: Provider[] }>('/api/v1/auth/providers')
      .then((data) => setProviders(data.providers))
      .catch(() => setError('Failed to load login options'));
  }, []);

  const handleLogin = async (providerType: string) => {
    setLoading(providerType);
    setError(null);

    try {
      const data = await api.post<{ authorization_url: string }>(
        '/api/v1/auth/login',
        {
          provider: providerType,
          redirect_uri: `${window.location.origin}/auth/callback`,
        }
      );

      window.location.href = data.authorization_url;
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Login failed');
      setLoading(null);
    }
  };

  // Handle OAuth callback
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const code = params.get('code');
    const state = params.get('state');

    if (code && state) {
      api
        .post<{
          access_token: string;
          account: {
            id: string;
            display_name: string;
            primary_email: string;
            avatar_url?: string;
            providers: string[];
          };
        }>('/api/v1/auth/callback', { code, state })
        .then((data) => {
          setAuth(data.access_token, data.account);
          window.location.href = '/';
        })
        .catch((err) => {
          setError(err instanceof Error ? err.message : 'Authentication failed');
        });
    }
  }, []);

  return (
    <div class="min-h-screen flex items-center justify-center bg-background p-4">
      <div class="w-full max-w-md">
        {/* Logo */}
        <div class="text-center mb-8">
          <div class="w-16 h-16 bg-primary rounded-2xl flex items-center justify-center mx-auto mb-4">
            <span class="text-3xl font-bold text-white">A</span>
          </div>
          <h1 class="text-2xl font-display font-bold text-foreground">
            Admin Portal
          </h1>
          <p class="text-foreground-secondary mt-2">
            Sign in to access the admin dashboard
          </p>
        </div>

        <div class="bg-background-secondary border border-border rounded-xl p-6">
          {error && (
            <div class="mb-4 p-3 bg-error/10 border border-error/20 rounded-lg text-error text-sm">
              {error}
            </div>
          )}

          <div class="space-y-3">
            {providers.map((provider) => (
              <button
                key={provider.type}
                onClick={() => handleLogin(provider.type)}
                disabled={loading !== null}
                class="w-full flex items-center justify-center gap-3 px-4 py-3 bg-background-secondary border border-border rounded-lg font-medium text-foreground hover:bg-background-tertiary hover:border-border-hover transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {loading === provider.type ? (
                  <div class="animate-spin rounded-full h-5 w-5 border-b-2 border-primary" />
                ) : (
                  <>
                    {provider.type === 'google' && <GoogleIcon />}
                    {provider.type === 'line' && <LineIcon />}
                    Continue with {provider.name}
                  </>
                )}
              </button>
            ))}

            {providers.length === 0 && !error && (
              <div class="text-center py-4 text-foreground-secondary">
                Loading login options...
              </div>
            )}
          </div>
        </div>

        <p class="text-center text-sm text-foreground-tertiary mt-6">
          Admin access only. Unauthorized access is prohibited.
        </p>
      </div>
    </div>
  );
}

function GoogleIcon() {
  return (
    <svg class="w-5 h-5" viewBox="0 0 24 24">
      <path
        fill="#4285F4"
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
      />
      <path
        fill="#34A853"
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
      />
      <path
        fill="#FBBC05"
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
      />
      <path
        fill="#EA4335"
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
      />
    </svg>
  );
}

function LineIcon() {
  return (
    <svg class="w-5 h-5" viewBox="0 0 24 24" fill="#06C755">
      <path d="M19.365 9.863c.349 0 .63.285.63.631 0 .345-.281.63-.63.63H17.61v1.125h1.755c.349 0 .63.283.63.63 0 .344-.281.629-.63.629h-2.386c-.345 0-.627-.285-.627-.629V8.108c0-.345.282-.63.627-.63h2.386c.349 0 .63.285.63.63 0 .349-.281.63-.63.63H17.61v1.125h1.755zm-3.855 3.016c0 .27-.174.51-.432.596-.064.021-.133.031-.199.031-.211 0-.391-.09-.51-.25l-2.443-3.317v2.94c0 .344-.279.629-.631.629-.346 0-.626-.285-.626-.629V8.108c0-.27.173-.51.43-.595.06-.023.136-.033.194-.033.195 0 .375.104.495.254l2.462 3.33V8.108c0-.345.282-.63.63-.63.345 0 .63.285.63.63v4.771zm-5.741 0c0 .344-.282.629-.631.629-.345 0-.627-.285-.627-.629V8.108c0-.345.282-.63.627-.63.349 0 .631.285.631.63v4.771zm-2.466.629H4.917c-.345 0-.63-.285-.63-.629V8.108c0-.345.285-.63.63-.63.348 0 .63.285.63.63v4.141h1.756c.348 0 .629.283.629.63 0 .344-.281.629-.629.629M24 10.314C24 4.943 18.615.572 12 .572S0 4.943 0 10.314c0 4.811 4.27 8.842 10.035 9.608.391.082.923.258 1.058.59.12.301.079.766.038 1.08l-.164 1.02c-.045.301-.24 1.186 1.049.645 1.291-.539 6.916-4.078 9.436-6.975C23.176 14.393 24 12.458 24 10.314" />
    </svg>
  );
}
